...
Wave 6.5（2026-05-09 v0.2 新增；T-3X-0 完成后启动；T-3.10 前置）:
   T-3X-1   [B]   ADR-030 立项 + AestheticPreference schema 落地 + prompt hook（依赖 T-3X-0 产出 AESTHETIC_PREFERENCES.md v0.1）
   ↓ PR merge 后 Wave 7 才能启动

Wave 7（实测期；A 阶段实测；不走完整 ABC，只走"实测 + 验收报告"）:
   T-3.10   [A]   完成标志实测（作者跑一周 ≥ 10 场景；0 critical + [A] ≥ 60% + Wilson CI + 场景集声明依赖图；R3.X 不强制；**v1.0.1 新增依赖 T-3X-1 PR merge + T-3X-0 commit**）
