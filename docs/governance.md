...
- ~~事件触发 git hook / GitHub Action 跑 GPT critique——L2 + L3 critique 全部手动跑性价比更高~~（**v0.6 修订**：B 阶段可由会话经中转站 API 驱动跨模型评审，见 §13；git hook / GitHub Action 形态仍不启用）
...
- ~~不要尝试搭 git hook / GitHub Action 自动化 critique——按当前频率手动跑性价比更高~~（**v0.6 修订**：B 阶段自动化改由 §13 规范——会话内 API 驱动，不走 git hook / GitHub Action）
...
- **2026-07-08 v0.6**：作者授权 B 阶段自动化（触发 = 作者以进度为由主动 revisit；PR #82 A→B 空转 6 天实证瓶颈在阶段间手动接力）。修订点：**新增 §13**（B 阶段可由会话经中转站 API 驱动跨模型评审 + C 自动衔接/可先行 + 验收后自动合并 + 作者触点收敛为"效果验收 + 架构授权"两类 + 手动通道保留）；§4 不启用清单第 2 条、§5 不要做第 1 条加删除线标注被 §13 取代。ABC 结构 / cross-LLM 要求 / L2 验收闸 / 安全阀硬闸不变；主链仍不接 routines。落地模式 = 作者当日对话明示授权 + L2 会话直落（参 v0.4.2 措辞同步级破例先例；本次属工作方式实质变更，授权语句 = 作者选定"中转站 API 跑 GPT 审（推荐）"并知悉"选了即视为授权修订治理文档"）。**默认行为**：B 阶段默认 API 自动跑（作者可随时手动加审）；L2 验收过关默认直接 merge。
...
## 13. B 阶段自动化（v0.6 新增，作者 2026-07-08 授权）

> **触发**：作者以进度为由主动发起 revisit（"ABC 阶段能弄成自动化的吗"）。原 §4/§5 "critique 手动跑性价比更高"的前提（任务频率低、作者带宽富余）已不成立——实测 T-3P-0（PR #82）A 阶段完成到 B 阶段启动空转 6 天，阶段间的作者手动接力成为主要延迟源。本节取代 §4/§5 中"不自动化 critique"的相关条目；**ABC 三阶段结构、cross-LLM 要求、L2 验收闸本身全部不变**。

### 13.1 B 阶段执行方式扩展

- 除作者手动起 Codex 会话外，B 阶段可由 L2 / 执行会话**经中转站 API 驱动跨模型评审**：打包 PR diff + 任务规格 + 模块规则 → 套 [/docs/REVIEW_PROMPT_CODE_GPT.md](REVIEW_PROMPT_CODE_GPT.md) 模板（API 交付形态：上下文由工具打包注入，评审模型只产报告文本）→ 调用中转站 → 报告落 `/docs/reviews/<ISO_DATE>_<task>_review.md` 并 push main 独立 commit（沿 §10 第 7 条）。
- **审查模型必须非 Claude 系**（cross-LLM 独立性是硬要求）；模型经 `.env` 的 `REVIEW_MODEL` 指定（GPT 系为默认）。
- 工具载体：`tools/cross_review.py`（v1 只产报告文件，commit/push 由调用会话执行）。

### 13.2 C 阶段自动衔接 + C 可先行

- B 报告产出后直接喂回原分支会话（或同分支新会话）修复，无需作者摆渡。
- **C 可先行**：已知的 L2 深审 findings / 作者已拍板的改动可在 B 报告到达前先修——B 审的是最终 diff，独立性不受损，且避免两轮修复。

### 13.3 验收后自动合并 + 作者触点收敛

- L2 验收过关 → 直接 merge + 删分支（作者全局 git 收尾授权的既有默认），不再等作者去 GitHub 操作。
- 作者触点收敛为两类：① **concrete 效果验收**（样例 / 预览 / 可玩产物——按大白话 + 玩家/编剧视角呈现，直接贴进对话）；② **架构级授权**（/schema、L1 文档、ADR、破例、跳 BC）。
- **保留手动通道**：作者随时可对任一 PR 手动加跑 Codex 会话；地基 / schema 级任务作者可要求在自动审之外叠加手动审（不互斥）。

### 13.4 边界（不变项）

- L1→L2→L3 主链仍不接 routines / 定时任务（§1 核心结论不变）；本节自动化只发生在**会话内**、由 L2 或作者触发。
- B 阶段绝不跳过（攒批 ≠ 跳过，§10.6 不变）；安全阀硬闸（§10.7）不变——自动审查中发现 /schema 等禁区 diff 同样 🔴 停批。
- 评审调用计入 cost_log；费用异常由 §4 的 5b 周对账兜底。
