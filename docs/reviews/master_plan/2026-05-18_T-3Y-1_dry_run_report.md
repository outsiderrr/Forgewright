...
酒馆后间只点着一盏煤油灯，杯架的影子落在湿木桌上。露西把托盘靠到墙边，指节还扣着边沿；角落那桌的西装男人换了坐姿，鞋尖朝着这边。她从围裙里抽出半截折皱名片，又用拇指盖住姓氏下方的小字。「莱特不只去大学和我这里。他有一套晚上用的名字，账本、钥匙、信都分开放。」她停了一下，视线掠过门口。「姓维克的人递过这张卡。有人听见这个姓，就会先看出口在哪儿。」露西把名片压在杯底下。「还有个乡下小屋，不在主路上。他说那里只放旧书，可我见过壁炉砖缝里有新灰。」

### Options（选项；4 个）

- **opt_continue_press**: 我还要那张名片。
- **opt_warn_about_vick**: 我会让你避开维克。
- **opt_press_for_cache**: 我想知道小屋位置。 *(检定条件 condition: `{"op": "gte", "path": "relationship.lucy.trust", "value": 2}`)*
- **opt_betray_lucy_to_eyes**: 我去和那桌人谈谈。
...
|---|---|---|
| **信息密度（information_density）** | **8.93** | `{"narration_char_count": 193, "option_count": 4, "quote_count": 3, "formula": "(chars/50)*0.5 + options*1.0 + quotes*1.0", "raw_base_before_clamp": 8.93}` |
| **白描合规度（baimiao_compliance）** | **10.00** | `{"narration_char_count": 193, "evaluation_word_count": 0, "figurative_word_count": 0, "violation_density_per_100_chars": 0.0, "formula": "10 - (eval+figurative)/(chars/100) * 2"}` |

### 2.2 Anti-pattern detector flags（程序化检测）

- **AP-7** @ `narration`：narration 含第三人称转述模式 '他说'——信息属于 NPC 的应让 NPC 自己说，旁白抢 NPC 台词违反 A1 反馈 v0.1 §2 AP-7
  - 触发片段：「下小屋，不在主路上。他说那里只放旧书，可我见过壁炉砖缝里有新灰。」」
...
| **input_tokens** | 3966 |
| **output_tokens** | 1195 |
| **实际成本（actual_cost_usd）** | $0.0223 |
| **LLM 调用耗时（elapsed_sec_llm）** | 24.56 s |
| **总耗时（elapsed_sec_total）** | 24.56 s |
| **finish_reason** | `stop` |
| **开始时间（started_at）** | 2026-05-18 19:31:25 |
...
```json
{
  "started_at": "2026-05-18 19:31:25",
  "model_id": "gpt-5.5",
  "steps": [
    {
      "step": "env_loaded",
    ...
    {
      "step": "budget_charged",
      "record_id": "49eb481181ad4484992fd02dd61e31e3",
    ...
    },
    {
      ...
      "input_tokens": 3966,
      "output_tokens": 1195,
      "actual_cost_usd": 0.022272,
      "elapsed_sec_llm": 24.5592200756073,
      "finish_reason": "stop"
    },
    {
      "step": "evaluation_complete",
      "anti_pattern_flags": 1,
      "rubric_information_density": 8.93,
...
```json
{
  "node_id": "node_3_info_offer",
  "type": "dialogue",
  "narration": "酒馆后间只点着一盏煤油灯，杯架的影子落在湿木桌上。露西把托盘靠到墙边，指节还扣着边沿；角落那桌的西装男人换了坐姿，鞋尖朝着这边。她从围裙里抽出半截折皱名片，又用拇指盖住姓氏下方的小字。「莱特不只去大学和我这里。他有一套晚上用的名字，账本、钥匙、信都分开放。」她停了一下，视线掠过门口。「姓维克的人递过这张卡。有人听见这个姓，就会先看出口在哪儿。」露西把名片压在杯底下。「还有个乡下小屋，不在主路上。他说那里只放旧书，可我见过壁炉砖缝里有新灰。」",
  "speaker_ref": "char_lucy",
  "location_ref": "scene_inn",
  "on_enter_effects": [
    {
