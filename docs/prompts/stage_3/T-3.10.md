...
> | Wave | 7（实测期；跨多日多次会话）|
> | 依赖 | T-3.5 + T-3.6a + T-3.6b + T-3.4 全部 PR merge **+ T-3.X1（ADR-030 + schema + prompt hook）PR merge + T-3.X0（AESTHETIC_PREFERENCES.md v0.1）已 commit**（2026-05-09 v0.2 新增）|
...
- 审美层 [A]ccept rate：**≥ 60% pilot + Wilson 95% CI 报告**（如 6/10 → CI 27%-86%；不用单点百分比伪装稳定；基于已结构化 /docs/AESTHETIC_PREFERENCES.md + /schema/aesthetic_preference.schema.json 字段集**真作 gate**——不再悬空阈值；2026-05-09 v0.2 新增）
- 实测吞吐：1 周 ≥ 10 场景
...
- /docs/AESTHETIC_PREFERENCES.md（2026-05-09 v0.2 新增；T-3.X0 产出；审美锚点字段集源 — T-3.10 运行前必须 v0.1 已 commit）
- /docs/DECISIONS.md ADR-030（AestheticPreference schema）+ ADR-020 v0.2（PR-A 已落地；阶段 2/3/4 三阶段 [A] gate 口径）
- /docs/reviews/master_plan/2026-05-09_aesthetic_layer_decision_v0.1.md v0.2 §6.2（审美层决策 v0.2；T-3.X0/X1 前置工作流）
