# STAGE_2_TASKS.md — 阶段 2 任务清单

> 本文件是阶段 2 任务清单的正式 source-of-truth。所有 L3 执行会话按 §8 paste-ready prompt 启动。
>
> 历史草稿见 /docs/reviews/master_plan/2026-05-03_STAGE_2_TASKS_v1.0_draft.md（含 v0.1.1 → v1.0 修订记录 + GPT-5.5 critique 整合）。

日期：2026-05-03 · 版本：v1.0 · 来源：L2 整合规划师产出（claude/musing-fermi-f6bfd3 worktree）+ GPT-5.5 cross-LLM critique 整合 + 2026-05-03 L1 Wave 2 校准

---

## 0. 文档说明

本文件 = 阶段 2 任务清单 source-of-truth。整合自 v0.1.1 草稿 + 2026-05-03 GPT-5.5 cross-LLM critique（19 finding：🔴 5 / 🟡 10 / 🟢 4）+ 2026-05-03 L1 Wave 2 校准（"全选推荐"路径）。

- 完整修订记录见 §12（v0.1.1 → v1.0 对照表）
- 跨边界事项见 §13（X1 / X2 / X3）
- 历史草稿（保留作审计）：/docs/reviews/master_plan/2026-05-03_STAGE_2_TASKS_v1.0_draft.md
...
STAGE_2_TASKS.md 落地：由 X3 L3 执行会话（worktree claude/naughty-satoshi-7fd67f）从 v1.0 草稿搬运（commit hash 待 PR merge 时回填）
...
- **作者另起做的事**：
  1. 审 v1.0 草稿（本任务即 X3 落地）（本文件）+ §12 修订记录 + §13 跨边界清单 + §10 自评 4 弱点
