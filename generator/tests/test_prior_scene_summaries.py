...
def test_truncate_preserves_chapter_boundary_with_recent_4():
    ...
    assert reason == "kept_recent_with_boundary"
...
def test_summary_source_hash_stable():
    """Same input → same hash; deterministic for sidecar audit."""
    s = _summary("scene_a", summary="prose", key_state_paths=["world.x"])
    assert _summary_source_hash(s) == _summary_source_hash(s)
...
def test_compute_token_metrics_over_cap_caps_count_and_records_reason():
    ...
    assert metrics.truncation_reason == "kept_recent_only"
