...
- `narration:` 的值从冒号后开始，**允许多行**，直到下一个 key 行或下一个 `[node: ...]` 为止。
- `dialogue:` 块每行以 `- ` 开头，**裸正文不带引号包裹**（「」/""都不要）；说话人不用写——本场对白统一是锁定配置里的那位 NPC（露西）。0 行时整个 `dialogue:` 块可省。
- `options:` 序号行形如 `1: 台词`；序号必须从 1 连续编到 N，不多不少。

### 2. beats 拍（单选项锁定微节点，id 形如 `{链}_b{i}`）：必交 `narration` + `continue`；`dialogue` 可选

本拍锁定要揭的线索（提示词包会随块给出，如：`莱特在希博公路北侧有一间旧测绘小屋。` /
`小屋不在主路旁，要从酒馆后方的土路绕过去。`）——正文必须把它们揭出来，且不得提前揭后面拍的线索。
...
dialogue:
  - 好，你别抬高声音，我就说方位。
  - 北边，希博公路北侧，他有间旧测绘小屋。别走主路，从酒馆后头那条土路绕。
continue: 到了以后找什么？
```

- `continue:` = 玩家把对话推进到下一拍的一句短话或动作（≤20 字），只此一条，不编号。
...
1. [E1 missing_node] 节点 `soft_private_line_b3`
   ...
   「露西不知道铁盒里是什么，或者她声称不知道。」「小屋钥匙可能藏在雨桶底下。」）。

2. [E4 option_count_mismatch] 节点 `opening` 的 options
   期望：序号 1..3 连续完整（锁定选项数 = 3）
   实际：交了 1: / 2: / 4:（缺 3，多 4）
   修改指引：把第三个选项的序号改回 3，选项总数保持 3 条，不得增删。
...
| E6 | unknown_key | 不认识的 key（含错位块：如 end 带 options） |
| E7 | empty_text | key 在但正文为空 |
| E8 | parse_error | 无法归属任何 key 的游离行 |

边界判定口径（逐 case 定死在 `format_spec.py`）：`options:` 块**整体缺失** = E5；
块在但序号缺 / 多 / 不连续 = E4；不该出现的块（end 带 options 等）= E6。
