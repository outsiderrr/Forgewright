...
> 验收未通过，未落地：schema 层 1 错（scene.json 结构不合 Schema）；一致性闭合 1 错（说话人未在花名册声明 / option_id 重复）；机械预检 2 错（effects / condition 形态违规）。这些都是编剧改不到的结构字段——问题出在管线或被手改过的 scene.json，别退回给编剧改正文；核对 design.json / 合并流程 / 是否有人手动动过 scene.json。
...
### 一致性层 · 闭合违规（说话人闭合 / option_id 唯一）：1
