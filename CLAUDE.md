...
**初心（北极星 / north star）**：做一款**让人能快速制作 RPG 游戏的软件**。当前要坚持的**路线（手段）**是"**AI 辅助设计可玩的对话结构 + 产出场景级写作提示词 + 校验回流正文**"——**正文本身由创作者（编剧）用自己选的模型写**（BYOM, bring your own model），本仓库不自建正文生成（ADR-039）。这是手段，不是终点；终点是让独立开发者用尽量小的工程与写作负担，把脑子里的故事做成可玩的 CRPG（电脑角色扮演游戏 / computer RPG）。
...
**阶段 3（内容生产流水线），2026-06-21 起按 ADR-039 重定义**：结构层（多 pass + 动态拓扑）已"有条件生产就绪"；当前主线 = 写作提示词包转向的核心闭环——**P-A 场景级写作提示词渲染器 + P-B 回流合并与验收落地**。正文不在本仓库生成（编剧 BYOM）。跨场景连续性先做便宜版；幕级打包 / 手动 loop / 费工连续性 / 文风资产重打包推到核心闭环验证后。

运行时永远无 LLM（架构共识 4）不变。详见 ADR-039 + `/docs/reviews/master_plan/2026-06-21_pivot_to_writer_prompt_pack.md`。
